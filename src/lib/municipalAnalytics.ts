...
export function calculateTrafficLoad(population: number, roadNetwork: Record<string, RoadSegment>) {
  ...
  Object.values(roadNetwork).forEach(road => {
    const capacityPerLane = ROAD_CAPACITY_PCU_PER_LANE_HR[road.type as keyof typeof ROAD_CAPACITY_PCU_PER_LANE_HR] || ROAD_CAPACITY_PCU_PER_LANE_HR.local;
    networkCapacityPCU += road.lanes * capacityPerLane;
  });
